<template>
  <section class="page-card page-grid">
    ...
    <div class="form-grid">
      ...
      <el-button type="primary" @click="create" :disabled="!avatarStore.currentAvatarId">{{ t('agents.create') }}</el-button>
    </div>
    <div class="list-grid">
...
const create = async () => {
  ...
  await workspace.createAgent(avatarStore.currentAvatarId, { name: name.value, role_prompt: rolePrompt.value, permissions: ['task:run'] })
...
const toggleStatus = async (agentId: string, agentStatus: string) => {
  ...
  await workspace.updateAgentStatus(avatarStore.currentAvatarId, agentId, nextStatus)
